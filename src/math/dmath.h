...
//TODO: replace all of this with turns/tau
...
#define HALF_SQRT3 0.86602540378f 
...
inline f32 dsin(f32 x) {
    return sinf(x);
}

inline f32 dcos(f32 x) {
    return cosf(x);
}

inline f32 dtan(f32 x) {
    return tanf(x);
}

inline f32 dacos(f32 x) {
    return acosf(x);
}

inline f32 dsqrt(f32 x) {
    return sqrtf(x);
}

inline f32 dabs(f32 x) {
    return fabsf(x);
}

inline u32 dabs(i32 x) {
    return x < 0 ? x * -1 : x;
}

inline i32 RoundF32ToI32(f32 val) {
    return roundf(val);
}

inline i32 RoundF32ToU32(f32 val) {
    return (u32)(val + 0.5f);
}

static i32 Floor(f32 val) {
...
inline f32 Lerp(f32 src, f32 dest, f32 t) {
    f32 result = dest*(1-t) + t*src;
    return result;
}
...
inline b32 InRangeClosedU(u32 value, u32 min, u32 max) {
    b32 result = (value >= min && value <= max);
    return result;
}

inline b32 InRangeLeftClosedU(u32 value, u32 min, u32 max) {
    b32 result = (value >= min && value < max);
    return result;
}

inline b32 InRangeRightClosedU(u32 value, u32 min, u32 max) {
    b32 result = (value > min && value <= max);
    return result;
}

inline b32 InRangeOpenU(u32 value, u32 min, u32 max) {
    b32 result = (value > min && value < max);
    return result;
}

inline b32 InRangeClosedI(i32 value, i32 min, i32 max) {
    b32 result = (value >= min && value <= max);
    return result;
}

inline b32 InRangeLeftClosedI(i32 value, i32 min, i32 max) {
    b32 result = (value >= min && value < max);
    return result;
}

inline b32 InRangeRightClosedI(i32 value, i32 min, i32 max) {
    b32 result = (value > min && value <= max);
    return result;
}

inline b32 InRangeOpenI(i32 value, i32 min, i32 max) {
    b32 result = (value > min && value < max);
    return result;
}
...
DINLINE Vec3 Mat4MultVec3(Mat4 mat, Vec3 vec) {
    Vec3 result = {};
    f32* data = mat.data;
...
DINLINE Vec4 Mat4MultVec4(Mat4 mat, Vec4 vec) {
    Vec4 result = {};
    f32* data = mat.data;
...
DINLINE Mat4 Mat4Orthographic(f32 left, f32 right, f32 bottom, f32 top, f32 nearClip = 0.1f, f32 farClip = 100.0f){
    Mat4 result = Mat4Identity();
...
DINLINE Mat4 Mat4Perspective(f32 fov_degrees, f32 aspect_ratio, f32 near_clip = 0.1f, f32 far_clip = 100.0f){
    ...
    //NOTE: these two are flipped from what i originally thought because im under the assumption of column vectors
    result.data[14] = -1.0f;
...
DINLINE Mat4 Mat4Scale(f32 scale){
    Mat4 result = Mat4Identity();
    result.data[0] = scale;
    result.data[5] = scale;
...
DINLINE Mat4 Mat4RST(Mat4 rotation, Mat4 scale, Mat4 translate) {
    Mat4 st = Mat4Mult(scale, translate);
    Mat4 rst = Mat4Mult(rotation, st);
    Mat4 result = Mat4Transpose(rst);
    return result;
}
...
inline Rect2 RectCenterHalfDim(Vec2 center, Vec2 half_dim) {
    Rect2 result = {};
...
inline b32 IsInRectangle(Rect2 rect, Vec2 test) {
    b32 result = test.x >= rect.min.x && test.x < rect.max.x && test.y >= rect.min.y && test.y < rect.max.y;
    return result;
}
